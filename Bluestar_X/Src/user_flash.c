...
/****************************************************************************
 Function: FLASH_If_Erase
 Purpose: This function does an erase of all user flash area.

 Input:	start: start of user flash area
 Return value: FLASHIF_OK : user flash area successfully erased
...
uint32_t FLASH_If_Erase_User(uint32_t start,enmBootMemorySegment memorySegment)
{
  ...
  else if(memorySegment == enmBOOTSEGMENT_Y_JUMP)
  {
	  ...
	  //pEraseInit.PageAddress = FLASH_X_START_ADDR;
	  //NbrOfPages = (FLASH_X_END_ADDR - FLASH_X_START_ADDR)/FLASH_PAGE_SIZE;
